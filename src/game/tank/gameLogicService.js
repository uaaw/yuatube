...
class GameLogicService {
    ...
    gameTick(clientData, socket, currentClientDatas) {
        ...
        if(clientData.lastHeartbeat < currentTime - config.maxLastHeartBeat){
            console.log(`Kicking player ${clientData.tank.screenName}`);
    ...
    gameTickSpectator(clientData, socket) {
        ...
        if(clientData.lastHeartbeat < currentTime - config.maxLastHeartBeat){
            console.log(`Kicking spectator ${clientData.tank.screenName}`);
            this.kill(clientData, socket);
        }else{
    ...
    handleCollisionsOnTank(clientData, socket, currentClientDatas) {
        ...
        // 2 second invincibility period after spawning to prevent immediate death
        if(new Date().getTime() - clientData.tank.spawnTime < 2000) {
            return;
        }
        ...
        for(var objectInTankArea of objectsInTankArea){
            if(objectInTankArea.type === 'BULLET'){
                ...
                // Prevent self-kill - a player's own bullets cannot damage them
                if(bullet.ownerId === clientData.id) {
                    continue;
                }
                ...
                //remove bullet from the shooter's bullet list and quadtree
                if(playerIndex > -1) {
                ...
                // Always remove bullet from quadtree regardless of shooter status
                this.quadtree.remove(bullet.forQuadtree(), 'id');
    ...
    /*
     * Finds a safe spawn location on the game board, clear of walls, other tanks, and bullets.
     * Has a max attempt limit to prevent infinite looping.
     */
    static getSpawnLocation(quadtreeManager){
        ...
        outerLoop: while(attempts < maxAttempts){
            attempts++;

            //generate random x and y within the board, with margin from edges
            var x = Math.floor(margin + (Math.random() * (config.gameWidth - 2 * margin)));
            var y = Math.floor(margin + (Math.random() * (config.gameHeight - 2 * margin)));
        ...
        console.log('Could not find safe spawn location after ' + maxAttempts + ' attempts, using center of map');
        return {
